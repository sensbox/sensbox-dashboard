...
class DeviceConsole extends React .Component {
  ...
  async componentDidUpdate(prevProps) {
    ...
    if (current !== prevProps.current) {
      ...
      deviceMessagesSubscription.on('create', (message) => {
        const time = `${message.get("createdAt").toLocaleDateString()} ${message.get("createdAt").toLocaleTimeString()}`;
        const data = {
  ...
  render() {
    ...
    const { chartsData, deviceMsgData, deviceSensorsData, fromDate, toDate, timer, selectedRefresh, refreshMenuItems } = this.state;
